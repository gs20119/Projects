...
double e[ORDER]={ -19.0/720, 106.0/720, -264.0/720, 646.0/720, 251.0/720 };
...
void AdamsPC()
{
    if(xMem[ORDER-1] > X) return; step++;
    double delta = 0;
    for(int i=0; i<ORDER; i++) delta += d[i]*f(xMem[i],yMem[i]);
    for(int i=0; i<ORDER; i++) xMem[i] += h;
    for(int i=0; i<ORDER-1; i++) yMem[i]=yMem[i+1];
    yMem[ORDER-1] += h*delta; step += ORDER+1; delta=0; // prediction
    
    for(int j=0; j<REPEAT; j++){ // correction, you can repeat this
        for(int i=0; i<ORDER; i++) delta += e[i]*f(xMem[i],yMem[i]); 
        yMem[ORDER-1] = yMem[ORDER-2]+h*delta; step += ORDER+1; delta=0; 
